...
import { IStrategyClassifier } from "../lib/IStrategyClassifier.sol"; 
    ...
    function vote(uint256 ytId, uint256[] calldata strategyIds, uint256[] calldata weights) external nonReentrant {
        ...
        if (existing.expiry > block.timestamp) {
            ...
            if (lastAdded > 0 && block.timestamp - lastAdded < MIN_RESET_DURATION && timeLeft < MIN_RESET_DURATION) {
               ...
               revert("You cannot extend your vote due to the strategy being updated."); //freezed
                 ///📌
                ///@notice I added this revert only for demonstration in tests
                ///@notice This revert is equivalent to -------->    expiry = existing.expiry;
...
// یا باید کنترل دسترسی داشته باشه یا همیشه بیشتر از 30 روز بشه صدا زد 
